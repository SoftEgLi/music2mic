...
Usage: python extract_bn_160ms.py --input_dir /path/to/wavs --output_dir /path/to/bns

Output: BN at 40ms per frame (same as original 120ms BN).
"""
...
# JIT ASR parameters (160ms model: 19-frame window, 4 BN per call, stride=16)
BN_WINDOW = 19   # fbank frames per JIT call
BN_STRIDE = 16   # fbank advance per call → 40ms per BN frame
